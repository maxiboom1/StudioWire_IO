...
This repository contains the v0.2.5.5 React, TypeScript, Vite, Tailwind CSS, and shadcn/ui app. It runs entirely in the browser with local autosave and JSON import/export.
...
- `v0.2.5.5`: Fixed ledger getter purity, tightened device status typing, reduced user-created ID collision risk, corrected Cables filter clear behavior, optimized crosspoint candidate scans, and documented bidirectional reset behavior.
- `v0.2.5.4`: Added device-row routing control for TB front segments, displayed second-segment cable numbers after inline TB markers, and tightened cable label spacing.
