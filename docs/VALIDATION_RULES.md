...
- `view-line-port-missing`: a port endpoint must reference a port that still exists on its placed source device.
- `view-line-port-invalid`: a port endpoint must resolve to a rendered row of a standard-device placement; TB/rack/missing-source/empty-row anchors are invalid.
- `view-line-range-missing`: an I/O Range endpoint must reference an annotation that still exists in the same View.
