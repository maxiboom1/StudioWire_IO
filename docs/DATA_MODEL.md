...
Fix 4 refines the virtual alignment grid without adding persistent fields. The grid origin is 10 mm. At View device scale `s`, horizontal and vertical pitch are both `(50 * 92 / 940) * s` mm, matching one standard-device I/O row. Direct navigator drops, committed movement, coordinate edits, and keyboard movement use the same grid; Alt bypasses pointer snapping. Normal drops and moves choose the nearest fitting grid cell when the live block can fit. Changing the View device size preserves each placement's grid column/row while updating all device/TB scales; manual line waypoints and annotations remain at their absolute millimetre coordinates.
