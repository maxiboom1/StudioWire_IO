...
Version `0.2.9.02` renders placements as read-only live references. The `0.2.9.02-fix-2` UI maintenance label does not change that schema: standard View devices uniformly reduce the complete Device Workspace diagram, with passive endpoint stubs replacing crosspoint controls. The persisted `scale` field remains valid for compatibility and any existing value is rendered uniformly, but the current operator UI provides no resize gesture or Scale field and never changes it. Device/TB port labels, cable numbers, destination summaries, and rack contents are always resolved from current project records and are never copied into a View. The optional `labelOverride` is presentation-only. Moving, labeling, or removing a placement may mutate only the owning View's `placements` plus normal project stamps/change-log metadata; removing a placement also removes View lines attached to that placement. It cannot modify locations, folders, rack assignments, devices, port groups, ports, cables, endpoints, or numbering ledgers.
