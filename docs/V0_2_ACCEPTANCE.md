...
Version `0.2.9.07` retains the completed first View editor and fixes crosspoint navigation so normal connection edits preserve the selected device/TB workspace. New Project, Import Project, and Load Sample remain the only project lifecycle actions that reset navigation and transient View state. It does not support logical grouping, per-object placement resizing, View printing/export, prewire export, Excel export, Bartender export, Visio export, SVG/PDF document export, authentication, backend storage, database storage, or multi-user collaboration.
