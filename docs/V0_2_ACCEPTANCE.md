...
Version `0.2.9.08` retains the completed first View editor, adds endpoint reconnection and transactional bend editing through existing technical handles, and uses a shared live TB diagram in source and View workspaces. New allocations use three-digit minimum cable numbers while legacy numbers remain exact. It does not support logical grouping, per-object placement resizing, View printing/export, prewire export, Excel export, Bartender export, Visio export, SVG/PDF document export, authentication, backend storage, database storage, or multi-user collaboration.
