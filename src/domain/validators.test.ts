...
describe('validateProject terminal block rules', () => {
  it('accepts the sample terminal block foundation', () => {
    const issues = validateProject(structuredClone(sampleProject));

    expect(issues).toHaveLength(0);
    expect(sampleProject.terminalBlocks).toHaveLength(1);
    expect(sampleProject.terminalBlockPortGroups).toHaveLength(1);
    expect(sampleProject.terminalBlockPorts).toHaveLength(32);
  });

  it('reports a missing terminal block port endpoint', () => {
    const project = structuredClone(sampleProject);

    project.cables.push({
      id: 'cable-missing-tb-port',
      number: 'V-9000',
      prefix: 'V',
      index: 9000,
      status: 'connected',
      sourceEndpoint: { type: 'unknown', id: null, label: '' },
      destinationEndpoint: { type: 'tb_port', id: 'tb-port-missing', label: 'Missing TB Port' },
      labelTop: '',
      labelMiddle: 'V-9000',
      labelBottom: 'Missing TB Port',
      notes: '',
    });

    const codes = validateProject(project).map((issue) => issue.code);

    expect(codes).toContain('cable-linked-to-missing-tb-port');
  });

  it('accepts a cable endpoint referencing an existing terminal block port', () => {
    const project = structuredClone(sampleProject);
    const tbPort = project.terminalBlockPorts[0];

    project.cables.push({
      id: 'cable-existing-tb-port',
      number: 'V-9000',
      prefix: 'V',
      index: 9000,
      status: 'connected',
      sourceEndpoint: { type: 'unknown', id: null, label: '' },
      destinationEndpoint: { type: 'tb_port', id: tbPort.id, label: tbPort.label },
      labelTop: '',
      labelMiddle: 'V-9000',
      labelBottom: tbPort.label,
      notes: '',
    });

    const codes = validateProject(project).map((issue) => issue.code);

    expect(codes).not.toContain('cable-linked-to-missing-tb-port');
  });

  it('reports duplicate terminal block face and position records', () => {
    const project = structuredClone(sampleProject);
    const duplicatePort = {
      ...project.terminalBlockPorts[0],
      id: 'tb-port-duplicate-face-position',
    };

    project.terminalBlockPorts.push(duplicatePort);

    const codes = validateProject(project).map((issue) => issue.code);

    expect(codes).toContain('duplicate-terminal-block-face-position');
  });
});
