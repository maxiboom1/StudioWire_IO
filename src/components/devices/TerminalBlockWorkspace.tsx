...
export function TerminalBlockWorkspace({ device }: { device: Device }) {
  ...
  const rearGroupIds = new Set(
    portGroups.filter((group) => group.direction === 'rear').map((group) => group.id),
  );
  const frontGroupIds = new Set(
    portGroups.filter((group) => group.direction === 'front').map((group) => group.id),
  );
          ...
          <div className="terminal-block-panel" role="img" aria-label={`${device.name} terminal block panel`}>
            {pairs.map((pair) => (
              <div className="terminal-block-port" key={pair.index}>
                ...
                <span className="terminal-block-rear-label">{pair.rearConnection?.chainLabel || 'N/C'}</span>
                <span className="terminal-block-cable-number">
                  {pair.rearConnection?.cable?.number ?? ''}
                </span>
                ...
                <span className="terminal-block-cable-number">
                  {pair.frontConnection?.cable?.number ?? ''}
                </span>
              </div>
            ))}
          </div>
